...
exports.bookinstance_delete_post = function (req, res) {
	BookInstance.findById(req.body.instid).then((bookinstance) => {
		if (!bookinstance) {
			res.render('delete_bookinstance', {
				title: 'Delete book instance',
				bookinstance,
		...
		BookInstance.findByIdAndRemove(req.body.instid, (err) => {
			if (err) return next(err);
...
exports.bookinstance_update_get = function (req, res) {
	...
	objPromiseAll({ bookinstance, books })
		.then(({ bookinstance, books }) => {
			console.log(bookinstance.formated_date);
			if (!bookinstance) {
	...
	(req, res, next) => {
		...
		if (!errors.isEmpty()) {
			...
			BookInstance.findByIdAndUpdate(
				...
				bookinstance,
				{},
				function (err, theinst) {
					if (err) return next(err);
					res.redirect(theinst.url);
				}
			);
		}
	},
